...
vi.mock('../code-analysis.js', () => ({
  analyzeCode: () => ({ files: [], truncated: false, totalProjectTokens: 0, compressedTokens: 0, includedTokens: 0, filesAnalyzed: 0, filesIncluded: 0, duplicateGroups: 0 }),
}));
...
describe('collectFingerprint', () => {
  ...
  it('includes gitRemoteUrl, fileTree, existingConfigs, and codeAnalysis', async () => {
    ...
    expect(fp.codeAnalysis).toEqual({ files: [], truncated: false, totalProjectTokens: 0, compressedTokens: 0, includedTokens: 0, filesAnalyzed: 0, filesIncluded: 0, duplicateGroups: 0 });
  });
});
